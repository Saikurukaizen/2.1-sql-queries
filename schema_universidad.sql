...
SELECT d.nombre AS departamento, p.apellido1, p.apellido2, p.nombre AS profesor FROM persona p LEFT JOIN profesor pro ON p.id = pro.id_profesor LEFT JOIN departamento d ON pro.id_profesor = d.id WHERE tipo = 'profesor' ORDER BY d.nombre ASC, p.apellido1 ASC, p.apellido2 ASC, p.nombre ASC; 
---
SELECT DISTINCT d.nombre AS departamento FROM profesor pro JOIN asignatura a ON pro.id_profesor = a.id_profesor JOIN departamento d ON pro.id_departamento = d.id WHERE a.id_grado = 4;
-- 
-- Resol les seguents consultes utilitzant RIGHT JOIN i LEFT JOIN;
SELECT d.nombre AS departamento, p.apellido1, p.apellido2, p.nombre AS profesor FROM persona p LEFT JOIN profesor pro ON p.id = pro.id_profesor LEFT JOIN departamento d ON pro.id_departamento = d.id WHERE p.tipo = 'profesor' ORDER BY departamento ASC, p.apellido1 ASC, p.apellido2 ASC, p.nombre ASC;
SELECT p.nombre, p.apellido1, p.apellido2 FROM persona p LEFT JOIN profesor pro ON p.id = pro.id_profesor WHERE p.tipo = 'profesor' AND (pro.id_departamento IS NULL);
SELECT d.nombre AS departamento FROM departamento d LEFT JOIN profesor pro ON d.id = pro.id_profesor WHERE pro.id_profesor IS NULL;
-- 
SELECT nombre FROM asignatura WHERE id_profesor IS NULL;
SELECT DISTINCT d.nombre AS departamento FROM departamento d LEFT JOIN profesor pro ON d.id = pro.id_departamento LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor WHERE a.id IS NULL;
SELECT nombre FROM persona WHERE tipo = 'alumno';
SELECT nombre FROM persona WHERE tipo = 'alumno' AND year(fecha_nacimiento) = 1999;
SELECT d.nombre AS departamento, COUNT(pro.id_profesor) AS profesores FROM departamento d LEFT JOIN profesor pro ON d.id = pro.id_departamento GROUP BY d.nombre ORDER BY d.nombre ASC;
SELECT g.nombre AS grados, COUNT(a.id) AS asignaturas FROM grado g LEFT JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre ORDER BY asignaturas DESC;
SELECT g.nombre AS grado, COUNT(a.id) AS asignaturas FROM grado g LEFT JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre ORDER BY COUNT(a.id) > 40;
SELECT g.nombre AS grado, a.tipo, SUM(a.creditos) AS total_creditos FROM grado g JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre, a.tipo;
SELECT * FROM persona WHERE tipo = 'alumno' ORDER BY fecha_nacimiento DESC LIMIT 1;
SELECT p.nombre, p.apellido1, p.apellido2 FROM persona p JOIN profesor pro ON p.id = pro.id_profesor LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor WHERE p.tipo = 'profesor' AND pro.id_departamento IS NOT NULL AND a.id IS NULL;