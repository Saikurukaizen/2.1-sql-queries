...
SELECT nombre AS producto, precio AS euros, precio * 1.10 AS "dólares norte-americanos" FROM producto;
SELECT UPPER(nombre) AS nombre_maj, precio FROM producto;
SELECT LOWER(nombre) AS nombre_min, precio FROM producto;
SELECT nombre AS nom_fabricante, UPPER(LEFT(nombre, 2)) AS dos_primeros_caracteres FROM fabricante;
SELECT nombre, ROUND(precio, 0) AS precio_redondo FROM producto;
...
SELECT p.nombre, p.precio, f.nombre AS fabricante FROM producto p JOIN fabricante f ON p.codigo_fabricante = p.codigo WHERE p.precio >= 180 ORDER BY p.precio DESC, p.nombre ASC;
